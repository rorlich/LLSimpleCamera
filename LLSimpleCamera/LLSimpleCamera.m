...
- (instancetype)init
{
    return [self initWithOutputType:LLCameraOutputStill];
}

- (instancetype)initWithOutputType:(LLOutputType)outputType
{
    return [self initWithQuality:AVCaptureSessionPresetHigh position:LLCameraPositionRear outputType:outputType];
}
...
- (instancetype)initWithQuality:(NSString *)quality position:(LLCameraPosition)position outputType:(LLOutputType)outputType
{
    ...
    if(self) {
        [self setupWithQuality:quality position:position outputType:outputType];
    }
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    if (self = [super initWithCoder:aDecoder]) {
        [self setupWithQuality:AVCaptureSessionPresetHigh
                    ...
                    outputType:LLCameraOutputStill];
    }
    return self;
}
...
- (void)setupWithQuality:(NSString *)quality
              ...
              outputType:(LLOutputType)outputType
{
...
//-(void)setCaptureConnection:(AVCaptureConnection *)captureConnection {
//    _captureConnection = captureConnection;
//    dispatch_async(dispatch_get_main_queue(),^{
//        [self handleRotation:[[UIApplication sharedApplication] statusBarOrientation] duration:0];
//    });
//}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    @autoreleasepool {
        if ( self.delegate && self.isRecording)
